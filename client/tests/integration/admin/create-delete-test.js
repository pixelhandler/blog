...
test('Menu has link to create a new post', function () {
  ...
  visit(hyperlink(createPost)).then(function () {
    equal(currentPath(), 'admin.create', 'links to Admin page to create a post');
  });
});
    ...
    //click(createPost).then(function () {
      ...
      Ember.RSVP.all(promises).then(function () {
        click(buttons.save).then(function () {
          equal(currentPath(), 'admin.index', 'Successfully saving a new post redirects to Admin index');
